          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => setView(item.id)}
                className={`flex flex-col items-center justify-center gap-1.5 transition-all duration-300 w-20 ${
              ...
              >
                ...
                <span className={`text-[10px] font-black uppercase tracking-tighter transition-all ${
                  isActive ? 'text-blue-500 scale-100 opacity-100' : 'text-slate-500 opacity-80'
                }`}>
                  {item.shortLabel}
                </span>
                {isActive && (
